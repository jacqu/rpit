...
#define S_FUNCTION_NAME rpi_sfun_trex
/*<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<*/
...
static void mdlOutputs(SimStruct *S, int_T tid)
{
    ...
    const int8_T  *rpi_trex_port  = (const int8_T *)mxGetData(PARAM_DEF0(S));
