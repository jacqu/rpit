...
#define S_FUNCTION_NAME sfun_OSEK_GAI
/*<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<*/
...
#define OUTPUT_0_DTYPE       DYNAMICALLY_TYPED
...
{
    int16_T        *y0  = ssGetOutputPortRealSignal(S,0);
    ...
    const real_T  *gain  = (const real_T *)mxGetData(PARAM_DEF1(S));
    const real_T  *intercept  = (const real_T *)mxGetData(PARAM_DEF2(S));
    const real_T  *sign  = (const real_T *)mxGetData(PARAM_DEF3(S));
