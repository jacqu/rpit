 ...
 *
 * File: ext_work.c     $Revision: 1.1.6.8 $
 *
 * Abstract:
 *   
 */
...
#include "ext_qnx_work.h" /* includes all VxWorks headers */
...
//#ifdef VXWORKS
#ifdef QNX_OS
sem_t*  volatile  uploadSem = NULL;
sem_t*  volatile  commSem   = NULL;
sem_t*  volatile  pktSem    = NULL;
                             ...
                             sem_t          *startStopSem)
{
    ...
    sem_unlink("uploadSem");
    sem_unlink("commSem");
    sem_unlink("pktSem");
    uploadSem = sem_open("uploadSem", O_CREAT,S_IRWXG,1);
    commSem   = sem_open("commSem", O_CREAT,S_IRWXG,1);
    pktSem    = sem_open("pktSem", O_CREAT,S_IRWXG,1);

    rt_ExtModeInit();

    
    pktServerArg.ei = ei;
    pktServerArg.numSampTimes = numSampTimes;
    pktServerArg.stopReq = stopReqPtr;
    pktServerArg.numSampTimes = numSampTimes;
    
    extern_pkt_tid = tgtCreateThread("tExternPkt", rt_PktServer,
                                   &pktServerArg, priority+1, stack_size);

    
    extern_upload_tid = tgtCreateThread("tExternUpload", rt_UploadServer,
                                   (void *) &numSampTimes, priority, 2*32768);
