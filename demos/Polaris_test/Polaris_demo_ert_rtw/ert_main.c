...
static int_T rpi_wait_period(struct rpi_periodic_info *info)
{
  ...
  if (ret == -1) {
    rtmSetErrorStatus(Polaris_demo_M, "timer read error");
...
void rt_OneStep(void)
{
  /* Check for overun */
  if (OverrunFlag++) {
    rtmSetErrorStatus(Polaris_demo_M, "Overrun");
    return;
  }
...
int_T main(int_T argc, const char_T *argv[])
{
  ...
  /* Initialize model */
  Polaris_demo_initialize(1);
  rpi_make_periodic((uint_T)(0.01*1000000.0), &info);
  ...
  while ((rtmGetErrorStatus(Polaris_demo_M) == (NULL)) && !rtmGetStopRequested
         (Polaris_demo_M)) {
